...
class AlphaBetaPlayerWithCentralization(Player):
    ...
    def play(self, board):
        move, value = self.alphabeta(board, eval_function_points_central, True, self.levels - 1, 0)
        x = move[0]
        y = move[1]
        return x, y
...
def eval_function_points_central(board, move, is_player):
    CENTRALIZATION_MULTIPLIER = 0.1
    BOARD_SIZE = board.shape[0]
    MIDDLE_POINT = BOARD_SIZE / 2
    step_distance_to_center = abs(move[0] - MIDDLE_POINT) + abs(move[1] - MIDDLE_POINT)
    if is_player:
        points = stratego_game.calculate_points(board, move)
        points -= step_distance_to_center * CENTRALIZATION_MULTIPLIER
    else:
        points = -stratego_game.calculate_points(board, move)
        points += step_distance_to_center * CENTRALIZATION_MULTIPLIER
